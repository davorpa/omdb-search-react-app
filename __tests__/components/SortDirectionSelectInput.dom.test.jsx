...
describe('SortDirectionSelectInput component', () => {
  it('should render without problems', () => {
    render(<SortDirectionSelectInput />)
    expect(true).toBeTruthy()
  })

  it('should render with default options if no props are provided', () => {
    // Given
    const inputIdMatcher = expect.stringMatching(/-sort-dir$/)
    // When
    const { container } = render(<SortDirectionSelectInput />)
    // Then
    ...
    expect(label).toHaveTextContent(/Sort direction/i)
    expect(input).toBeInTheDocument()
    expect(input).toHaveAttribute('id', inputIdMatcher)
    expect(input).toHaveAttribute('name', 'sortDir')
    expect(input).not.toHaveAttribute('required')
    expect(input.querySelectorAll('option')).to.have.lengthOf(2) // asc, desc
  })
  ...
  it('should render with a CSS class equal to the className attribute', () => {
    ...
    const { container } = render(
      <SortDirectionSelectInput className={className} />
    )
    // Then
  ...
  it('should render with required attribute', () => {
    // Given
    // When
    const { container } = render(<SortDirectionSelectInput required />)
    // Then
  ...
  it('should render with custom name attribute', () => {
    ...
    const name = 'my-name'
    // When
    const { container } = render(<SortDirectionSelectInput name={name} />)
    // Then
    const input = findInputInside(container)
  ...
  it('should render with a custom formId attribute', () => {
    ...
    // When
    const { container } = render(<SortDirectionSelectInput formId={formId} />)
    // Then
    const input = findInputInside(container)
  ...
  it('should render with a custom label attribute', () => {
    ...
    // When
    const { container } = render(
      <SortDirectionSelectInput labelText={labelText} />
    )
    // Then
    const label = findLabelInside(container)
  ...
  it('should handle onValueChange event when a handler is provided', () => {
    ...
    // When
    const { container } = render(
      <SortDirectionSelectInput onValueChange={onValueChange} />
    )
    const input = findInputInside(container)
  ...
  it('should handle valueSetter when a handler is provided as function', () => {
    ...
    // When
    const { container } = render(
      <SortDirectionSelectInput name={name} valueSetter={valueSetter} />
    )
    const input = findInputInside(container)
    fireEvent.change(input, { target: { value } })
    // Then
